...
**Scaffold:** run `pnpm exec create-dflow-app` from this repo (after `pnpm install`), or use `pnpm dlx` once the package is published. Examples and flags: [packages/create-dflow-app/README.md](./packages/create-dflow-app/README.md). Maintainer verification: `pnpm run verify:create-dflow-app` copies each starter to a temp dir and runs install + build; **missing runtimes (Go, JDK, Python, etc.) are skipped by default** so the command succeeds on a partial laptop. For a **full** check that fails if any runtime is missing, use `pnpm run verify:create-dflow-app:strict` (or `VERIFY_STRICT=1`).
