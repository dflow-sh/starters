...
**Heroku / Dokku:** those stacks run **`npm start`** after build. This template defines **`start`** → `vite preview` on `0.0.0.0` with **`${PORT:-4173}`** so the platform’s `PORT` is honored when set (otherwise **4173**).
